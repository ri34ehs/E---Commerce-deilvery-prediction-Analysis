...
# Make sure 'late_shipment_predictor.pkl' is in the same folder
try:
    model = joblib.load('late_shipment_predictor.pkl')
except FileNotFoundError:
    st.error("Model file not found. Please ensure 'late_shipment_predictor.pkl' is in the same folder.")
    st.stop()
...
st.markdown("This app predicts whether an e-commerce shipment will be late based on its characteristics. This is a Proof of Concept based on the model we built.")
...
if st.button("Predict Delay Risk", type="primary"):
    ...
    # The order of columns must exactly match the order the model was trained on
